...
path4 = "World_Data.xlsx"
path5 = "population_forest.xlsx"

# Data set 1 - CO2 per capital emission
...
list = ["Year", "Australia", "China", "India", "United States", "Russian Federation", "Japan",
        "Iran, Islamic Rep.", "Korea, Rep.", "Saudi Arabia", "Indonesia", "Singapore"] 
...
def print_stats(data):
    ...
    the calculate the skewness and kurtosis. Produce average 
...
ax.set_xlabel("Year")
ax.set_ylabel("CO2 emissions from gaseous fuel consumption (% of total)", fontsize=9)
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.title("CO2 Emission from gaseous fuel consumption")
plt.show()

my_list = [13, 40, 55, 106, 109, 112, 119, 126, 202, 205, 208, 251]
df_country_interest = df_country.iloc[my_list] #.reset.index=True #, inplace = True   # remember stop value needs to be one larger
df_country_interest.reset_index(drop=True, inplace = True)
...
ax[0].set_ylabel("CO2 emissions from gaseous fuel consumption (% of total)", fontsize = 9) 
ax[0].set_title('1995')
...
df_country_interest1995 = df_country_interest.loc[["Australia", "Germany", "Japan", "Saudi Arabia", "Singapore"] , [1995 , 2015]]
df_country_interest1995.plot.bar()


# Data Set 5 - World Population and Forest Area
df = pd.read_excel(path5)
corr = np.corrcoef(df["Forest area (% of land area)"], df["World Population"])
print(corr)
fig, ax = plt.subplots(figsize=(4, 3))
ax.scatter(df["Forest area (% of land area)"], df["World Population"], s=50, facecolor='C0', edgecolor='k')
...
#sns.set(font_scale=2) # font size 3
